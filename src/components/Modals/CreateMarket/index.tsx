...
const CreateMarket = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => any }): JSX.Element => {
    const { colorMode } = useColorMode();
    const values = useContext(ContractContext);
    const { contracts, dispatch } = useContext(ContractContext);

    const factoryContract = useContract(addresses[KOVAN_ID].marketFactory, MBMarketFactoryContract.abi, true);
    const [loading, setLoading] = useState<boolean>(false);

    const [marketEventName, setMarketEventName] = useState<string>('');
    const [arbitrator, setArbitrator] = useState<string>('0xd47f72a2d1d0E91b0Ec5e5f5d02B2dc26d00A14D');

    const [marketOpeningTime, setMarketOpeningTime] = useState<number>(Date.now() / 1000);
    const [marketLockingTime, setMarketLockingTime] = useState<number>(Date.now() / 1000 + 86400);
    const [marketResolutionTime, setMarketResolutionTime] = useState<number>(Date.now() / 1000 + 172800);
    ...
    const onSubmit = async (e: FormEvent) => {
        ...
        if (marketOpeningTime >= marketLockingTime || marketLockingTime >= marketResolutionTime) {
    ...
    const removeOption = (index: any) => () => setOptions(options.filter((s: any, sidx: any) => index !== sidx));
