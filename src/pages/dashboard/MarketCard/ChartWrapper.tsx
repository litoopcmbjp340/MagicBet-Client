...
export default function Chart({ marketContract }: { marketContract: Contract }) {
    ...
    useEffect(() => {
        let isStale = false;
        if (!isStale) marketContract.on('ParticipantEntered', (address: any) => setNewAddress(address));
        ...
        (async () => {
            ...
            if (!isStale) {
                ...
                for (let i = 0; i < outcomes.length; i++) {
                    ...
                    const amountOfBetsForOutcome = await marketContract.getBetAmountsArray(i);
                    ...
                    const betTimestampsOnOutcome = await marketContract.getTimestampsArray(i);
                    ...
                    outcomeBetsAndTimestamp.forEach((item: any) => {
                        ...
                        else if (openingTime + 64800 < point && point < openingTime + 129600)
                            //@ts-ignore
                            newData1[outcomeName] = newData1[outcomeName] + item.amount;
                        else if (openingTime + 129600 < point && point < openingTime + 194400)
                            //@ts-ignore
                            newData2[outcomeName] = newData2[outcomeName] + item.amount;
                        else if (openingTime + 194400 < point && point < openingTime + 259200)
                            //@ts-ignore
        ...
        <Flex align="center" direction="column">
            ...
            <ButtonGroup isAttached={true} size="xs">
                <Button
                    ...
                    isDisabled //! undisable once we better understand an event's timeline
                >
                    24H
                </Button>
                <Button
