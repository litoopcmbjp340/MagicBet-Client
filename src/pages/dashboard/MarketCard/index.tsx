...
const CountDown = ({ startDate }: { startDate: number }): any => {
    ...
    function getTimeUntil(realStartDate: any) {
        const time = Date.parse(realStartDate) - Date.parse(new Date() + '') - 7 * 60 * 60 * 1000;
        setTime(time);
    ...
    return time <= 0 ? '-' : <>{`${add0(days)}:${add0(hours)}:${add0(minutes)}:${add0(seconds)}`}</>;
};

const MarketCard = ({ marketContractAddress }: any) => {
    const marketContract = useContract(marketContractAddress, MBMarketContract.abi, true);
    const daiContract = useContract(addresses[KOVAN_ID].tokens.DAI, IERC20.abi, true);
            ...
            if (!!marketContract) {
                try {
                    ...
                    if (numberOfOutcomes.toNumber() !== 0 && !isStale) {
                        const numberOfOutcomes = (await marketContract.numberOfOutcomes()).toNumber();
                        let newOutcomes = [];
    ...
    useEffect(() => {
        (async () => {
            let isStale = false;
            if (!!marketContract && !isStale && marketContract.provider) {
                try {
    ...
    const placeBet = async (e: FormEvent): Promise<void> => {
      ...
      setDaiApproved(true);
        ...
        const increaseByFactor = (number: any) => number.mul(BigNumber.from(120)).div(BigNumber.from(100));
        ...
        const estimatedGas = await marketContract!.estimateGas.placeBet(indexOfChoice, formatted, {
    ...
    return !prompt ? null : (
        <>
            <Box bg={bgColor8[colorMode]} borderRadius="0.5rem" m="0 1.5rem" px="0.5rem" pb="1.5rem">
                <Flex
                ...
                >
                    <Stat>
                        ...
                        <StatNumber fontFamily="monospace">{shortenAddress(marketContract!.address)}</StatNumber>
                    </Stat>
                    ...
                    <Stat>
                        ...
                        <StatNumber fontFamily="monospace">
                            {marketResolutionTime && <CountDown startDate={marketResolutionTime} />}
                        </StatNumber>
                    </Stat>
            ...
            <SettingsModal settingsModalToggle={settingsModalToggle} marketContract={marketContract} />
        </>
    );
};
